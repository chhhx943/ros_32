...
TEST(AdapterCoreTest, ReadOnlyCanWarningKeepsFeedbackAndOdometryDegraded)
{
  ...
  BackendHealth warning{
    true, false, true, true, 0x000BU, TransportErrorClass::Warning};
  core.on_backend_health(warning, 0.01);

  EXPECT_EQ(core.status().state, AdapterState::Degraded);
  EXPECT_TRUE(core.status().safe_stop_active);
  EXPECT_FALSE(core.status().fault_latched);
  EXPECT_EQ(core.status().fault_code, 0U);
