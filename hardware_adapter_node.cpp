...
public:
    ...
    declare_parameter("max_position_delta_m", 0.20)})
  {
    ...
    read_only_ = declare_parameter("read_only", false);
...
private:
  ...
  void update()
  {
    ...
    // read_only: suppress ALL CAN control-frame TX (safe stop keepalive
    // included) so phase-1 verification is strictly read-only on the bus.
    // Feedback polling and wheel-odometry publication are unaffected.
    if (!read_only_) {
